...
- 桌面端账号登录、注册、退出登录、演示账号都通过 backend `/api/auth/*` 完成
- 当前前端身体信号展示以 `HRV + squeeze pressure + presence` 为主，不依赖 BPM
- 好友页数据由 backend `/api/friends/*` 提供；当前已支持展示账号、推荐列表、排行榜和加好友动作
...
返回展示账号的 backend session，用于账号页和好友页演示。
...
### `GET /api/friends/ranking?limit=20`
