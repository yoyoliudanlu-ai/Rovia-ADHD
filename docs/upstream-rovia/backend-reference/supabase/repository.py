...
class SupabaseRepository:
        ...
        *,
        ...
        try:
            ...
            if "trigger_source" not in str(error):
                raise
            fallback_payload = {k: v for k, v in payload.items() if k != "trigger_source"}
            resp = self.client.table("focus_sessions").insert(fallback_payload).execute()
        return self._first(resp, payload)
    ...
    def finish_focus_session(
        ...
        *,
        ...
        start_time = self._parse_datetime(self._first(existing, {}).get("start_time"))
        now_dt = datetime.now(timezone.utc)
        if start_time and now_dt < start_time:
            now_dt = start_time + timedelta(seconds=1)
    ...
    def get_focus_sessions(self, *, user_id: str, limit: int = 20) -> list[dict]:
        try:
            ...
            resp = (
                self.client.table("focus_sessions")
                .select("id, user_id, start_time, end_time, duration_minutes, status, is_active")
                .eq("user_id", user_id)
                .order("start_time", desc=True)
                .limit(max(1, int(limit)))
                .execute()
