...
# ── 配置（从环境变量读，方便替换） ──────────────────────────────
WRISTBAND_NAME = os.getenv("WRISTBAND_DEVICE_NAME", "ZhiYa-Wristband")
# 兼容旧变量名；这里表示“优先使用的数据输入特征”，不再局限于 notify。
WRISTBAND_INPUT_UUID = os.getenv(
    "WRISTBAND_INPUT_CHAR_UUID",
    os.getenv("WRISTBAND_NOTIFY_CHAR_UUID", "beb5483e-36e1-4688-b7f5-ea07361b26a8"),
)
...
SQUEEZE_NAME = os.getenv("SQUEEZE_DEVICE_NAME", "ZhiYa-NieNie")
...
class BleRunner:
    ...
    def disconnect_device(self, which: str):
        if which not in {"wristband", "squeeze", "all"}:
            return

        async def _disconnect_task():
            if which in {"wristband", "all"}:
        ...
        if self._loop and self._loop.is_running():
            self._loop.call_soon_threadsafe(
                lambda: asyncio.ensure_future(_disconnect_task())
            )

    def reconnect_device(self, which: str):
        if which not in {"wristband", "squeeze", "all"}:
            return
