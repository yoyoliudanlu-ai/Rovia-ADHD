...
@router.get("")
def list_todos():
    return _repo().list_todos(user_id=_user_id())
...
def update_todo(todo_id: str, body: UpdateTodoRequest):
    ...
    return _repo().update_todo(user_id=_user_id(), todo_id=todo_id, **patch)


@router.delete("/{todo_id}", status_code=204)
def delete_todo(todo_id: str):
    _repo().delete_todo(user_id=_user_id(), todo_id=todo_id)
